...
interface IStake {
    ...
    function estimateRewards(address _account) external view returns (uint);
}
...
contract CompoundLens {
    ...
    function estimateFTP(IFtp comp,ComptrollerLensInterface comptroller,IFarm farm,IStake stake, address account) external returns(FTPRewards memory) {
        ...
        uint accrued = comptroller.compAccrued(account);
        uint total = add(accrued, loanBalance, "sum comp total");
        uint loanFTP = sub(total, balance, "sub allocated");
        farm.claimRewards(account);
        uint farmBalance = comp.balanceOf(account);
        uint farmFTP = sub(farmBalance, loanBalance, "sub allocated");
        uint stakeFTP = stake.estimateRewards(account);
        return FTPRewards({
            loanReward:loanFTP,
            farmReward:farmFTP,
            stakeReward:stakeFTP
        });
    }
